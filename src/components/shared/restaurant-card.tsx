...
export function RestaurantCard({
  ...
  return (
    <Link
      ...
      className={cn(
        "press block",
        carousel ? "w-[264px] shrink-0" : "w-full",
        !r.open && "opacity-60"
      )}
    >
      <div className="relative overflow-hidden rounded-2xl">
        <PhotoTile
          ...
          src={r.image}
          alt={r.name}
          className={carousel ? "h-36 w-full" : "h-44 w-full"}
        />
        {r.offer ? (
        ...
        <span className="absolute right-2.5 top-2.5 grid size-8 place-items-center rounded-full bg-surface text-ink shadow-[var(--shadow-sm)]">
      ...
      <div className="px-0.5 pt-2">
        <h3 className="truncate text-[16px] font-extrabold leading-tight tracking-tight">
          {r.name}
        </h3>
