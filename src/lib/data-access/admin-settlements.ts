...
export interface SettlementPreview {
  ...
  refundsRecovered: number;
  /**
   * This batch's own total, before any outstanding correction (0046). netPayable
...
/** 0046. A missing relation (42P01), not a missing column — probed the same way. */
...
export async function settlementEstimateFor(input: {
  ...
  // land against one of its orders (0046) will see a smaller estimate here —
...
async function writeSettlement(
  ...
  // Outstanding corrections (0046) — refunds recovered against orders whose
...
/* ------------------------------------------------------------------
 * Settlement corrections (0046) — recovering a refund from a vendor that
 ...
 *
 * Degrades to zero on a database without 0046 applied — the same shape as
...
export async function voidSettlement(input: {
  ...
  // Any correction this settlement had absorbed (0046) goes back to
