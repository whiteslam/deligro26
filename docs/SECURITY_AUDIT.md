...
| 2026-08-26 | Change review | Migration 0045 (authored as 0043) — app release config + `GET /api/app-version` | §1 and §4 walked. **The design for this change asserted that `platform_settings` is plain public-read with no column-level grant list. It is not, and building it to that assumption would have shipped the feature broken.** 0032 revoked SELECT from `anon`/`authenticated` and replaced it with an explicit column allowlist (to hide `vendor_commission_pct`), and 0034 re-ran that block for `commission_gst_pct`. The allowlist is materialised from `information_schema.columns` at migration time, so it is a snapshot: the eight new columns would have existed, been written by the admin form, and been unreadable by the anon-scoped client the public route uses. Worse than a hard failure — `availableGroups()` treats a non-42703 error as transient and drops the group without latching, so the route would have answered from `DEFAULT_SETTINGS` indefinitely, telling every app it was current, with no error and a Settings screen that saved successfully. **Measured, not assumed:** replayed 0015 → 0032 → 0034 → 0043 on a throwaway Postgres 16 (the file is now 0045; see the 2026-08-26 merge row below); the denial for an ungranted column is SQLSTATE **42501**, and `isMissingColumn` matches 42703 only. It therefore re-runs the allowlist block, the same way 0034 re-ran 0032's. Verified after replay: all eight columns granted to `anon`, both restricted columns still withheld (count 0), `anon` can read the new columns under the 0015 `using (true)` policy, migration replays clean twice. **Nothing sensitive added to a publicly-readable table:** four integers, two URLs and two free-text notes — no PII, no money-routing data, no credential, no statutory identifier. The data is public by requirement, not by accident: an app must ask "am I too old to run?" before it has a session, and a version code and download URL both ship inside every APK already. **No new server action** — the existing `saveSettingsAction` (`requireRole("admin")`) writes them; no policy changed, so writes remain `is_admin()`. **New endpoint, read-only:** `GET /api/app-version` is unauthenticated by design and unrate-limited by §4's own scope (no write path). Inputs are validated, not cast — `parseReleaseApp` rejects anything but `rider`/`customer` rather than defaulting to one, and `parseVersionCode` uses `Number()` + `isSafeInteger` because `parseInt("1.9")` is 1 and `parseInt("12abc")` is 12, both of which would silently answer a question nobody asked. **One control added beyond the design (§6, M-1 class):** the APK URL is https-only (`safeApkUrl`), enforced when saved AND when served. Stricter than `safeExternalHref`, which permits http because a banner points at a web page; what is at the end of this URL gets *installed* by a phone already told to trust sideloaded builds, so over http any network between the rider and the file can substitute a different APK undetectably — the update channel would be a better attack than anything it updates. Serve-side enforcement matters because the write-side check cannot cover a row written before it shipped or posted straight to PostgREST. **Fails open, deliberately and narrowly:** an unreadable settings row yields the `DEFAULT_SETTINGS` shape where every code is 1, so no real build compares as behind and nothing is force-updated. That is availability, not authorization — AGENTS.md §2 governs access control, and this route grants nothing; failing closed here would brick a fleet on a transient database fault. The floor is clamped to the latest in three independent places (CHECK constraint, admin action, and the answer itself) because a floor above the latest is the one setting with no way back, and the constraint does not exist on a pre-0045 database. Covered by `npm run test:app-version` (47 offline cases) and six route cases in the E2E smoke. |
| 2026-08-26 | Change review | Merging `office`: migration 0044 — alert sounds — was missing the `platform_settings` column allowlist re-run | §1 walked on incoming code, not just on my own. **0044 adds six columns to `platform_settings` and never re-runs the column allowlist, so the feature it ships is silently dead on any database that has run 0032/0034.** Same trap as the 0045 row above, found because that row had just been written. The vendor kitchen board and the rider board read these through `getSettings()` on the RLS-scoped client as `authenticated`; an ungranted column denies with **42501**, `isMissingColumn()` matches **42703** only, so `availableGroups()` treats it as transient, drops the group without latching, and serves `DEFAULT_SETTINGS` forever. The visible symptom is nothing at all: an admin uploads a custom sound, hears it play from the Test button (which plays the form's own URL client-side, never touching the column), saves successfully, and no vendor or rider ever hears anything but the default chime. **Measured, not reasoned:** replayed 0015 → 0032 → 0034 → 0044 on a throwaway Postgres 16 — the anon grant count stayed at 20, `authenticated` was denied 42501 on `vendor_alert_sound_preset`, and `business_name` still read fine as a control. **Fixed in 0044 itself** rather than left to be fixed as a side effect of 0045, which happened to repair it by re-running the allowlist over all current columns: that link is invisible, and anyone deploying the office branch alone — or applying migrations in a different order — would get the broken state with no signal. The added block is the same `do $$ ... public_cols ... $$` shape as 0032/0034/0045, enumerating what is PUBLIC so a later column is excluded by default. Re-verified standalone on a clean database: 26 columns granted to `anon` (20 + the 6 new), alert-sound columns readable by `authenticated`, `vendor_commission_pct` and `commission_gst_pct` still denied 42501, migration replays clean twice. **Nothing sensitive was exposed by the fix** — a preset name from a four-value CHECK, a URL in an already-public bucket, and the original filename of an admin's own upload. The bucket policies, the admin-only write path and the magic-byte check in `uploadAlertSound` are unchanged. |
| 2026-09-04 | Change review | Migration 0047 (authored as 0045): COD collection record, cash handover chain, operational expenses | §1 walked. Two new columns on `deliveries` (`cod_collected_amount`, `cod_collected_at`), both written only by `advanceDelivery()` on the rider's own completed delivery, never by a client-supplied value. Two new tables, `cod_handovers` and `operational_expenses`, both with RLS enabled and: a select policy for `is_admin()` or `current_role() = 'manager'`, and no insert, update or delete policy for any authenticated role. `anon`/`authenticated` are revoked all privileges and `authenticated` is re-granted select only, matching the read policy; every write goes through the service role from `cod-handovers.ts` and `operational-expenses.ts`, reached only behind `requireRole(["manager", "admin"])` in `src/app/manager/cash/actions.ts`, the same shape as `vendor_settlements` (0028). Who can now see a column they could not see before: a manager, who could not previously read either table because neither existed, now sees rider and admin profile ids and rupee amounts tied to cash handling and Deligro's operating costs. This is the intended widening: the confirmed business rule requires a manager to be able to record and review this chain, and a manager already reads every active order and every rider's identity through the existing 0023 grants, so no new class of data reaches that role. Neither table carries a bank account, a card number or a credential. No function added; no existing policy, grant or function touched. Both tables are additive only, so a database without 0047 degrades to "the columns and tables do not exist," which `driver-orders.ts` and the new data-access files handle the same way the rest of this codebase handles a missing migration: `recordCodCollection()` probes the column and swallows the write rather than failing the delivery, and the `/manager/cash` and `/admin/cash-ledger` screens are new routes with no prior behaviour to regress. |
| 2026-09-04 | Change review | Migration 0048 (authored as 0046): settlement corrections, and the markSettlementPaid staleness re-check | §1 walked. One new table, `settlement_corrections`, RLS enabled: a select policy for `is_admin()` only (unlike 0047's tables, a manager is not admitted, matching that a manager has no role anywhere else in the vendor-settlement schema, 0023), and no insert, update or delete policy for any authenticated role. `anon`/`authenticated` revoked all privileges, `authenticated` re-granted select only; every write goes through the service role from `admin-settlements.ts`, called only from `refunds.ts`'s `decideRefund()`, itself reached only behind `requireRole("admin")` in the refunds Server Action. Who can now see a column they could not see before: only admin, who already reads every settlement and every refund through existing 0028/0001 grants — no new class of data reaches anyone. No existing policy, grant or function touched; `writeSettlement()`, `voidSettlement()`, `previewSettlement()`, `settlementEstimateFor()` and `markSettlementPaid()` in `admin-settlements.ts` all gained logic but none gained a new authorization surface — every one of them was already reachable only from an admin-gated caller, and the new reads/writes go through the same `createAdminClient()` those functions already used. `markSettlementPaid()` additionally re-reads `refunds` for every order in the draft being paid, which is a superset of data that function's own caller (admin) can already read directly in `/admin/refunds`. A database without 0048 applied degrades honestly: `outstandingCorrectionsFor()` probes for the missing relation (PGRST205/42P01, a new `isMissingTable()` helper alongside the existing `isMissingColumn()` in `schema-probe.ts`) and returns zero, so every settlement function behaves exactly as it did before this migration until it is applied. |
